...
其中运算符 `eq`是判断是否相等的操作，除此之外，还有 `ne`、 `lt`、 `gt`、 `and`、 `or`、`contains`等运算符。
...
helm create nginx创建一个helm的nginx应用，里面包含的目录文件有
- charts
- Chart.yaml
- templates
...
```
helm install web --dry-run /root/nginx #可以使用--dry-run检查语法是否正确

helm install web3 --set replicas=1 /root/nginx # 可以使用--set来设置变量的值，优先于values.yaml设置的变量值
```